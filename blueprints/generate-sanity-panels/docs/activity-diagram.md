...
flowchart TD
    ...
    UserInput["In: clarify outline\ntarget + brief + domain\n+ credentials + target requirements"]
    ...
    subgraph S7 [7 Draft]
        ...
        HttpPatch["HTTP patch draft"]
        DraftOut["Out: draftPatch previewUrl panelsUsed[]"]
        S7In --> SchemaRun --> MapPanels --> HttpPatch --> DraftOut
    end
    ...
    subgraph S9 [9 Publish]
        PublishRun["HTTP publish_sanity_document.py"]
...
| Stage | In | Out |
|---|---|---|
| Definition | `target`, `brief`, `domain`, credentials (+ `productId` if product) | `definitionBundle`, `domainSnapshot`, `template` |
