...
| Draft | `references/7-draft/` + `http-sanity.md` | `draftPatch`, `previewUrl`, `panelsUsed[]` |
...
- Max 2 identical panel `_type` per surface
- Never `markdown` or `richText` panel `_type`
- Product strips: never jobs panels (`featuredOrganisationJobs`, `organisationJobSearch`, `appJobs`)
- No self-surface links in CTAs or `edenLink`
- Generated/custom panel art = Sanity refs; catalogue covers = picker IDs (no re-upload)
- Never preview on `www.eden.co.uk`
- Product strips: title + subtitle per block; 6–9 block module budget; customer-facing copy only (`references/shared/copy-voice.md`)
- Audit requires browser preview + deterministic checklist — no Gate 8 without `auditResult.json` + `previewEvidence`
