...
## CRE hesitation gap (required)

Synthesize **`topHesitations[]`** (exactly 2–3 items) from FAQ/review/audience research. Load `references/shared/cre-conversion.md`.

Each item:

```json
{
  "hesitation": "Is this too academic for a seeker?",
  "source": "Customer review themes / credited press",
  "stripBeat": "audience-pathways or faq-hub"
}
```

- `stripBeat` = block id or beat type in Stage 5 (`audiencePathways`, `faqObjections`, `worksComparison`, etc.)
- Do not invent hesitations — ground in findings only
- Stage 5 maps each hesitation in `hesitationGapMap`
